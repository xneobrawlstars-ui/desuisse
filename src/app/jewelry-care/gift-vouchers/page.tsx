  ...
  return (
    <>
      ...
      <div style={{ background: '#f7f3ee', padding: '64px 40px', textAlign: 'center', borderBottom: '1px solid #e8e0d4' }}>
        ...
        <p style={{ fontFamily: 'Montserrat', fontSize: 13, color: '#888', maxWidth: 500, margin: '16px auto 0', lineHeight: 1.9 }}>{t.subtitle}</p>
            ...
            return (
              ...
              >
                ...
                <div style={{ background: 'linear-gradient(135deg, #1a0a0a 0%, #3d1a1a 100%)', padding: '40px 28px', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <p style={{ fontFamily: 'Montserrat', fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#c9a84c', marginBottom: 16, position: 'relative' }}>GIFT VOUCHER</p>
                  <p style={{ fontFamily: 'Cormorant Garamond', fontSize: '2.4rem', fontWeight: 300, color: '#fff', position: 'relative', letterSpacing: '0.05em' }}>{v.label}</p>
                  <p style={{ fontFamily: 'Montserrat', fontSize: 10, color: '#888', marginTop: 20, position: 'relative', letterSpacing: '0.15em' }}>DeSuisse Luxury Jewellery</p>
                  {/* Heart button */}
                ...
                <div style={{ padding: '20px 20px 16px' }}>
                  <h3 style={{ fontFamily: 'Cormorant Garamond', fontSize: '1.3rem', fontWeight: 400, color: '#1a0a0a', marginBottom: 4 }}>
                    Gift Voucher
                  </h3>
                  <p style={{ fontFamily: 'Montserrat', fontSize: 13, color: '#666', marginBottom: 16 }}>
                    {t.from} {v.label}
                  </p>
                  <button onClick={() => { addToCart(fakeProduct, 1, '', '', v.amount); }}
                    style={{ width: '100%', padding: '12px', background: isHovered ? '#c9a84c' : '#1a0a0a', color: '#fff', border: 'none', fontFamily: 'Montserrat', fontSize: 10, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer', transition: 'background 0.2s' }}>
                    {t.addToCart}
                  </button>
                </div>
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32 }}>
            {t.steps.map((step, i) => (
              <div key={i} style={{ textAlign: 'center', padding: '32px 24px', border: '1px solid #e8e0d4' }}>
                ...
                <h3 style={{ fontFamily: 'Cormorant Garamond', fontSize: '1.3rem', fontWeight: 400, color: '#1a0a0a', marginBottom: 10 }}>{step.title}</h3>
                <p style={{ fontFamily: 'Montserrat', fontSize: 12, color: '#888', lineHeight: 1.8 }}>{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
