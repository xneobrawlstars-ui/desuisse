...
export default function QuickView({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  const displayPrice = currentVariant ? `${currentVariant.price.toLocaleString('de-DE')}.00€` : formatPrice(product);
  ...
  const handleAddToCart = () => {
    ...
    addToCart(product, qty, selectedVariant, selectedSize, unitPrice);
    onClose();
  };
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: '#fff', zIndex: 301, width: '90%', maxWidth: 880, maxHeight: '90vh', overflowY: 'auto', display: 'grid', gridTemplateColumns: '1fr 1fr', animation: 'qvSlideIn 0.25s ease', boxShadow: '0 20px 80px rgba(26,10,10,0.25)' }} className="qv-modal">
        ...
        <div style={{ padding: '40px 36px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
          ...
          <h2 style={{ fontFamily: 'Cormorant Garamond', fontSize: '2rem', fontWeight: 400, color: '#1a0a0a', marginBottom: 10 }}>{product.name}</h2>
          <p style={{ fontFamily: 'Cormorant Garamond', fontSize: '1.5rem', fontWeight: 500, color: '#1a0a0a', marginBottom: 20 }}>{displayPrice}</p>
          <div style={{ width: 40, height: 1, background: '#e8e0d4', marginBottom: 22 }} />

          {/* Material variants */}
          {hasVariants && (
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontFamily: 'Montserrat', fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#999', marginBottom: 10 }}>
                {t.material}{selectedVariant ? ': ' : ''}<span style={{ color: '#1a0a0a' }}>{selectedVariant}</span>
              </p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {product.materialVariants.map(v => (
                  <button key={v.name} onClick={() => setSelectedVariant(v.name === selectedVariant ? '' : v.name)} style={{ padding: '7px 14px', border: `1px solid ${selectedVariant === v.name ? '#1a0a0a' : '#e8e0d4'}`, background: selectedVariant === v.name ? '#1a0a0a' : '#fff', color: selectedVariant === v.name ? '#fff' : '#444', fontFamily: 'Montserrat', fontSize: 11, cursor: 'pointer', transition: 'all 0.15s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    <span>{v.name}</span>
          ...
          {product.sizes && product.sizes.length > 0 && (
            <div style={{ marginBottom: 22 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {product.sizes.map(s => (
                  <button key={s} onClick={() => setSelectedSize(s === selectedSize ? '' : s)} style={{ width: 44, height: 40, border: `1px solid ${selectedSize === s ? '#1a0a0a' : '#e8e0d4'}`, background: selectedSize === s ? '#1a0a0a' : '#fff', color: selectedSize === s ? '#fff' : '#444', fontFamily: 'Montserrat', fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s' }}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e8e0d4' }}>
              ...
              <span style={{ width: 36, textAlign: 'center', fontFamily: 'Montserrat', fontSize: 13, fontWeight: 500 }}>{qty}</span>
            ...
            <button onClick={handleAddToCart} style={{ flex: 1, padding: '14px 16px', background: '#1a0a0a', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'Montserrat', fontSize: 10, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', transition: 'background 0.2s' }}
          ...
          <div style={{ borderTop: '1px solid #f0ebe3', paddingTop: 14, marginTop: 'auto' }}>
            ...
            <p style={{ fontFamily: 'Montserrat', fontSize: 11, color: '#bbb', marginBottom: 8 }}><strong style={{ color: '#999' }}>{t.category}:</strong> {t.getCategoryLabel(product.category)}</p>
            <button onClick={() => { onClose(); router.push(`/product/${product.id}`); }} style={{ background: 'none', border: 'none', fontFamily: 'Montserrat', fontSize: 11, color: '#c9a84c', cursor: 'pointer', letterSpacing: '0.06em', textDecoration: 'underline', padding: 0 }}>
              {t.viewFull} →
            </button>
          </div>
        </div>
